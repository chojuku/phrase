...
<?php
    if(!$db = new PDO("sqlite:../phrase.db")){
       ...
       header("Location: index.php?err=0");//DB‚Ì–â‘è      
       exit;
   }

   if(!$_POST['newid'] ||!$_POST['newname'] || !$_POST['newpass1'] || !$_POST['newpass2']){
       header("HTTP/1.1 301 Moved Permanently");
       header("Location: index.php?err=1");//“ü—Í‘«‚è‚È‚¢      
       exit;
   } 
   elseif($_POST['newpass1']!=$_POST['newpass2']){
       header("HTTP/1.1 301 Moved Permanently");
       header("Location: index.php?err=2");//ƒpƒXƒ[ƒh•sˆê’v
       exit;      
   }
   else{
     $newid = $_POST['newid'];
   ...
   //  print $sql;
     ...
     if(!$flag){
       header("HTTP/1.1 301 Moved Permanently");
       header("Location: index.php?err=2");//–â‡‚¹Ž¸”s
       exit;      
     }
     ...
     if(!$flag){
       header("HTTP/1.1 301 Moved Permanently");
       header("Location: index.php?err=3");//–â‡‚¹Ž¸”s
       exit;      
     }
     else{
       //ƒƒOƒCƒ“‚µ‚¿‚á‚¤
